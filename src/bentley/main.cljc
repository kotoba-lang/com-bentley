(ns bentley.main
  "Kotodama WASM entrypoint for the Bentley clean-room actor (L5) — Clojure port.
  ...
  facts (`bentley.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bentley")
(def tier "L5")
...
(def entity-specs
  [{:entity "iModel"       :plural "imodels"       :id-prefix "bentley_imo"
    :fields [:name :description :iTwinId :state :createdDateTime :dataCenterLocation :initialized :displayName]
    :required [:name :description]
    :coerce {:initialized :bool}                :refs {}}
   {:entity "iTwin"        :plural "itwins"        :id-prefix "bentley_itw"
    :fields [:subClass :type :status :displayName :number :iTwinAccountId]
    :required [:subClass :type]
    :coerce {}                                   :refs {}}
   {:entity "Changeset"    :plural "changesets"    :id-prefix "bentley_cha"
    :fields [:index :displayName :description :parentId :pushDateTime :creatorId :briefcaseId :changesetGroupId :fileSize :state]
    :required [:index :displayName]
    :coerce {:index :int :briefcaseId :int :fileSize :int} :refs {}}
   {:entity "NamedVersion" :plural "namedversions" :id-prefix "bentley_nam"
    :fields [:displayName :changesetId :changesetIndex :name :description :createdDateTime :state]
    :required [:displayName :changesetIndex]
    :coerce {:changesetIndex :int}               :refs {}}
   {:entity "Connection"   :plural "connections"   :id-prefix "bentley_con"
    :fields [:displayName :description :iModelId :iTwinId :connectorName :authenticationType]
    :required [:displayName :description]
    :coerce {}                                   :refs {}}
   {:entity "Job"          :plural "jobs"          :id-prefix "bentley_job"
    :fields [:status :progress :createdDateTime :lastModifiedDateTime :iModelId :iTwinId :testId :changesetId]
    :required [:status :progress]
    :coerce {:progress :int}                     :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"bentley.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bentley-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bentley-compat/ready)
